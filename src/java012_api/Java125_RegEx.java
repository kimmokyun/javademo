...
// sn 변수에 포험되어잇으면 true 아니면 false을 리턴한다.

System.out.println(sn.matches(".*[a-zA-Z_*$].*")); // a~z,A~Z,($) :달라는 숫자

//sn 변수에 숫자 0-9가 포함되어 잇으면 true아니면 false을 리턴한다.
System.out.println(sn.matches(".*[0-9].*"));
System.out.println(sn.matches(".*[\\d].*"));
System.out.println(sn.matches(".*\\d.*"));     // 3개다 같은 의미


//sn 변수에 숫자 0-9가 포함되어 잇지 않으면 true 아니면 false
System.out.println(sn.matches(".*[^0-9].*")); // [^<not 의 의미를 함]

// sb 변수에 저장된 값이 2로 시작되면 true 아니면 false를 리턴한다.
System.out.println(sb.matches("2.*"));

//sb 변수에 저장된 값이 2로 시작하고 5로 끝나면 true 아니면 false
System.out.println(sb.matches("2.*5"));

//st변수에 저장된 값이 a이거나 b로 시작하고 임의문자가 5개이면 true 아니면 false
System.out.println(st.matches("[ab].{5}")); 

//st 변수에 지정된 값이 a이나 b로 시작하고 임의문자가 1~5개이며 cc로 끝나면 true 아니면 false리턴
st = "aetwefcc";
...
System.out.println(res);
		Pattern pt = Pattern.compile("best", Pattern.CASE_INSENSITIVE); //CASE_INSENSITIVE (대소문자 구분안함)
